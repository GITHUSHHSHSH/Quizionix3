...
const showSignInModeBtn = document.getElementById('showSignInModeBtn');
const showCreateModeBtn = document.getElementById('showCreateModeBtn');
const openCreateModeBtn = document.getElementById('openCreateModeBtn');
const openSignInModeBtn = document.getElementById('openSignInModeBtn');
...
const toggleCreatePasswordBtn = document.getElementById('toggleCreatePasswordBtn');
const forgotPasswordBtn = document.getElementById('forgotPasswordBtn');
const viewTosBtnSignIn = document.getElementById('viewTosBtnSignIn');
const viewTosBtnCreate = document.getElementById('viewTosBtnCreate');
const agreeTosBtn = document.getElementById('agreeTosBtn');
const tosStatusMsg = document.getElementById('tosStatusMsg');
...
let tosAcceptedForCreate = false;
...
const USER_STORE_KEY = 'quizionix_users_v1';
...
function isValidFatimaEmail(email) {
  return /^[a-zA-Z0-9._%+-]+@student\.fatima\.edu\.ph$/.test(email);
}

function setAuthMode(mode) {
  const createActive = mode === 'create';

  signInForm?.classList.toggle('hidden', createActive);
  createAccountForm?.classList.toggle('hidden', !createActive);
  showSignInModeBtn?.classList.toggle('active', !createActive);
  showCreateModeBtn?.classList.toggle('active', createActive);

  if (authHeading) authHeading.textContent = createActive ? 'Create your account' : 'Sign in to your account';
  if (authSubtitle) authSubtitle.textContent = createActive
    ? 'Use your Fatima University student email and set your username.'
    : 'Use your Fatima University student email to continue.';
...
function beginTosWait(seconds = 12) {
  if (!agreeTosBtn) return;
  let remaining = seconds;
  agreeTosBtn.disabled = true;
  agreeTosBtn.textContent = `I agree to the Terms of Service (wait ${remaining}s)`;
  tosAcceptedForCreate = false;
  if (tosStatusMsg) tosStatusMsg.textContent = '';

  const intervalId = window.setInterval(() => {
    remaining -= 1;
    if (remaining <= 0) {
      window.clearInterval(intervalId);
      agreeTosBtn.disabled = false;
      agreeTosBtn.textContent = 'I agree to the Terms of Service';
      return;
    }
    agreeTosBtn.textContent = `I agree to the Terms of Service (wait ${remaining}s)`;
  }, 1000);
...
function completeLogin(email, username = 'Student') {
  localStorage.setItem('userEmail', email);
  localStorage.setItem('userName', username);
  if (profileEmailEl) profileEmailEl.textContent = `${username} | ${email}`;
...
showSignInModeBtn?.addEventListener('click', () => setAuthMode('signin'));
showCreateModeBtn?.addEventListener('click', () => setAuthMode('create'));
openCreateModeBtn?.addEventListener('click', () => setAuthMode('create'));
openSignInModeBtn?.addEventListener('click', () => setAuthMode('signin'));

toggleSignInPasswordBtn?.addEventListener('click', () => {
  setPasswordVisibility(signInPasswordInput, toggleSignInPasswordBtn);
});

toggleCreatePasswordBtn?.addEventListener('click', () => {
  setPasswordVisibility(createPasswordInput, toggleCreatePasswordBtn);
});

viewTosBtnSignIn?.addEventListener('click', openTosModal);
viewTosBtnCreate?.addEventListener('click', openTosModal);
closeTosBtn?.addEventListener('click', closeTosModal);

acceptTosBtn?.addEventListener('click', () => {
  tosAcceptedForCreate = true;
  if (tosStatusMsg) tosStatusMsg.textContent = 'Terms of Service accepted.';
  closeTosModal();
});

agreeTosBtn?.addEventListener('click', () => {
  if (agreeTosBtn.disabled) return;
  tosAcceptedForCreate = true;
  if (tosStatusMsg) tosStatusMsg.textContent = 'Terms of Service agreed. You can now create your account.';
});

forgotPasswordBtn?.addEventListener('click', () => {
  const email = signInEmailInput?.value.trim().toLowerCase() || '';
  if (!isValidFatimaEmail(email)) {
    if (loginError) loginError.textContent = 'Enter your Fatima email first to request password help.';
    return;
  }
  const users = loadUserStore();
  if (!users[email]) {
    if (loginError) loginError.textContent = 'No account found for that Fatima email.';
    return;
  }
  if (questionsStatusEl) {
    questionsStatusEl.textContent = `Password reset message queued for ${email}.`;
    questionsStatusEl.classList.remove('error');
  }
});
...
document.addEventListener('click', event => {
  ...
  const control = target.closest(
    '.btn-main, .btn-ghost, .subjectBtn, .subjectStudyBtn, .menu-toggle-btn, .settings-theme-option, .choiceBtn, .sidebar ul li, .submenu li, .front-settings-btn, .settings-toggle-inline input, .settings-toggle input, .text-link-btn, .auth-switch-btn, .password-toggle'
  );
  if (!control) return;
...
signInForm?.addEventListener('submit', async event => {
  ...
  try {
    ...
    if (!user) {
      if (loginError) loginError.textContent = 'No account found. Create an account first.';
      return;
    }
    ...
    completeLogin(email, user.username || 'Student');
  } catch (_error) {
...
createAccountForm?.addEventListener('submit', async event => {
  ...
  if (username.length < 2) {
    if (loginError) loginError.textContent = 'Username must be at least 2 characters.';
    return;
  }

  if (!isValidFatimaEmail(email)) {
    if (loginError) loginError.textContent = 'Only Fatima student email is allowed.';
    return;
  }
  ...
  if (!tosAcceptedForCreate) {
    if (loginError) loginError.textContent = 'You must agree to the Terms of Service before creating an account.';
    return;
  }
  ...
  try {
    const users = loadUserStore();
    if (users[email]) {
      if (loginError) loginError.textContent = 'That Fatima email already has an account. Sign in instead.';
      return;
    }
    ...
    if (loginError) loginError.textContent = 'Could not create account in this browser.';
  }
});
...
async function initializeAdaptiveSystem() {
  setQuestionsStatus('Preparing quiz system...');
  if (startBtn) startBtn.disabled = true;
  subjectBtns.forEach(btn => { btn.disabled = true; });

  try {
    await engine.initialize();
    setQuestionsStatus('System ready.');
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (savedEmail && profileEmailEl) profileEmailEl.textContent = `${savedName} | ${savedEmail}`;
});
